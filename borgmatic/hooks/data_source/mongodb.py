...
def make_password_temporary_config_file(password, borgmatic_runtime_directory):
    ...
    logger.debug(f'Writing MongoDB password to temporary configuration file to {password_config_file.name}')
...
def make_password_config_file(database, password, borgmatic_runtime_directory):
    ...
    if password_transport == 'pipe':
        path = make_password_config_file_pipe(password)
    elif password_transport == 'file':
        path = make_password_temporary_config_file(password, borgmatic_runtime_directory)
...
def build_dump_command(database, config, password_config_file_path, dump_filename, dump_format):
    ...
    return (
        ...
        + (('--config', shlex.quote(password_config_file_path)) if password_config_file_path else ())
        + (
            (
...
def remove_data_source_dumps(
    ...
    dump.remove_data_source_dumps(make_password_config_file_path(borgmatic_runtime_directory), 'MongoDB password config files for', dry_run)
