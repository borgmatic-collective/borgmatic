...
MONITOR_STATE_TO_WATCHGOOSE = {
    monitor.State.START: 'start',
    monitor.State.FINISH: None,  # Watchgoose doesn't append to the URL for the finished state.
    monitor.State.FAIL: 'fail',
    monitor.State.LOG: 'log',
}

DEFAULT_PING_BODY_LIMIT_BYTES = 10000
HANDLER_IDENTIFIER = 'watchgoose'
TIMEOUT_SECONDS = 10
...
def initialize_monitor(hook_config, config, config_filename, monitoring_log_level, dry_run):
    ...
    we can send them all to Watchgoose upon a finish or failure state. But skip this if the
...
def ping_monitor(hook_config, config, config_filename, state, monitoring_log_level, dry_run):
    '''
    Ping the configured Watchgoose URL or UUID, modified with the monitor.State. Use the given
    configuration filename in any log entries, and log to Watchgoose with the giving log level.
    If this is a dry run, then don't actually ping anything.
    '''
    ping_url = (
        hook_config['ping_url']
        if hook_config['ping_url'].startswith('http')
        else f"https://watchgoose.com/{hook_config['ping_url']}"
    )
    dry_run_label = ' (dry run; not actually pinging)' if dry_run else ''

    if 'states' in hook_config and state.name.lower() not in hook_config['states']:
        logger.info(f'Skipping Watchgoose {state.name.lower()} ping due to configured states')
        return

    ping_url_is_uuid = re.search(r'\w{8}-\w{4}-\w{4}-\w{4}-\w{12}$', ping_url)

    watchgoose_state = MONITOR_STATE_TO_WATCHGOOSE.get(state)
    if watchgoose_state:
        ping_url = f'{ping_url}/{watchgoose_state}'

    if hook_config.get('create_slug'):
        if ping_url_is_uuid:
            logger.warning('Watchgoose UUIDs do not support auto provisionning; ignoring')
        else:
            ping_url = f'{ping_url}?create=1'

    logger.info(f'Pinging Watchgoose {state.name.lower()}{dry_run_label}')
    logger.debug(f'Using Watchgoose ping URL {ping_url}')
    ...
    if not dry_run:
        ...
        try:
            ...
            logger.warning(f'Watchgoose error: {error}')
